  ...
  componentWillMount() {
    axios.get(`/api/applications?id=${sessionStorage.getItem('token_id')}&netId=prk57`)
      .then((response) => {
        ...
        // this is really bad
        for (const opp in response.data) {
          for (const app in response.data[opp].applications) {
            const curApp = response.data[opp].applications[app];
            const curOpp = response.data[opp].opportunity;
            if (curApp !== undefined) {
              if (curApp.id === this.props.match.params.id) {
                ...
                axios.get(`/api/undergrads/la/${this.state.application.undergradNetId}?tokenId=${sessionStorage.getItem('token_id')}`)
                  .then((response) => {
                    ...
                    const transcriptIdText = response.data.transcriptId != null ? '' : response.data.transcriptId;
