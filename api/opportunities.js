    ...
    verify(token, (undergradNetId) => {
      ...
      undergradModel.findOne({ netId: undergradNetId }, (err, undergrad) => {
        ...
        if (undergrad1 === undefined || undergrad1 === null) {
          ...
          opportunityModel.find(timeRange)
            ...
            .exec((err, opportunities) => {
              ...
              if (urlLabId && urlLabId !== 'null') {
                debug('2');
                labModel.findById(urlLabId, (err, lab) => {
                  debug(lab);
          ...
          }).sort(sortOrderObj).lean().exec((err, opportunities) => {
            debug('length!!!!');
            debug(opportunities);
            if (!opportunities) {
              return res.send({});
...
app.post('/', (req, res) => {
  ...
  verify(token, (netIdActual) => {
    ...
    (err, studentsWhoMatch) => {
      for (const undergrad1 in studentsWhoMatch) {
        const msg = {
...
app.delete('/:id', (req, res) => {
  ...
  opportunityModel.findByIdAndRemove(id, (err, opportunity) => {
...
app.get('/:id', (req, res) => {
  ...
  verify(req.query.netId, (tokenNetId) => {
    ...
    opportunityModel.findById(req.params.id).lean().exec((err, opportunity) => {
      if (err) {
        debug(err);
        res.send(err);
      }
